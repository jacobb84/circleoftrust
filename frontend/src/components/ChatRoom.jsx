...
import { Send, Users, Clock, LogOut, Copy, Check, Shield, AlertTriangle, X, Smile } from 'lucide-react';
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-300">Entering secure room...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="h-screen flex flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-4">
            <div className="relative" ref={userListRef}>
              <button
                ...
                aria-label={`${onlineUsers.length} users online. Click to view list`}
              ...
              >
                ...
                <span className="text-slate-200">{onlineUsers.length}</span>
              </button>
                  ...
                  <div className="max-h-64 overflow-y-auto">
                    {onlineUsers.map((user, index) => (
                      ...
                      >
                        ...
                        <span className={`text-sm ${user === username ? 'text-teal-400 font-medium' : 'text-slate-200'}`}>
            ...
            <div 
              ...
              className={`flex items-center gap-2 text-sm px-3 py-1 rounded-full ${
                isExpiringSoon() ? 'bg-amber-500/20 text-amber-300' : 'bg-slate-800 text-slate-300'
              }`}
            >
              {isExpiringSoon() && <AlertTriangle className="w-4 h-4" aria-hidden="true" />}
            ...
            <button
              onClick={handleLeave}
              className="flex items-center gap-2 px-3 py-2 text-sm text-slate-300 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-all"
            >
      ...
      <main className="flex-1 overflow-hidden flex flex-col max-w-4xl mx-auto w-full">
        {announcements.length > 0 && (
          <div className="flex-shrink-0 px-4 pt-2" role="log" aria-live="polite" aria-label="Room announcements">
            {announcements.map((announcement) => (
              <div
                key={announcement.id}
                className="mb-2 px-4 py-2 bg-slate-800/80 border border-slate-700 rounded-lg text-center text-sm text-slate-300 animate-pulse"
              >
                {announcement.text}
              </div>
            ))}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-slate-400 text-sm mt-2">All messages are end-to-end encrypted.</p>
            </div>
          ) : (
            messages.map((msg) => (
              <div
              ...
              >
                <div className={`max-w-[70%] ${msg.isOwn ? 'order-2' : ''}`}>
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-xs font-medium ${msg.isOwn ? 'text-teal-400' : 'text-slate-300'}`}>
        ...
        <div className="flex-shrink-0 p-4 border-t border-slate-800 bg-slate-950/50">
          <form onSubmit={handleSend} className="flex items-stretch gap-3 relative">
            <div className="relative flex" ref={emojiPickerRef}>
              <button
                ...
                className="px-4 bg-slate-800/50 border border-slate-700 rounded-xl text-slate-300 hover:text-teal-400 hover:border-slate-600 transition-all flex items-center"
              >
            ...
            <input
              ...
              className="flex-1 px-4 py-3 bg-slate-800/50 border border-slate-700 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500 transition-all"
            />
            <button
              type="submit"
